...
const THIS_ROLE = 'pro' as const;
const OTHER_ROLE = 'admin' as const;

// PRO panel is on 5173, Admin panel is on 5174 (one port higher)
function getAdminPanelOrigin(): string {
  const { protocol, hostname, port } = window.location;
  const proPort = parseInt(port || '80', 10);
  const adminPort = proPort + 1;
...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (role === THIS_ROLE) {
        // ✅ PRO creds → go straight to PRO dashboard
        navigate('/', { replace: true });
      } else {
        // ✅ Admin creds entered here → cross-app redirect to Admin panel
        setRedirecting(true);
        const user: DemoUser = {
        ...
        const targetUrl = buildHandoffUrl(getAdminPanelOrigin(), user);
        setTimeout(() => {
        ...
        <div className="dn-card" style={{
          ...
          {redirecting && (
            <div style={{
              ...
              <p style={{ margin: 0, fontSize: '13px', color: '#E8894A', fontWeight: 600 }}>Redirecting to Admin Panel…</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label htmlFor="dn-pro-username" style={{ fontSize: '12px', fontWeight: 600, color: 'rgba(255,255,255,0.8)', letterSpacing: '0.05em' }}>USERNAME</label>
            ...
            {/* Password */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label htmlFor="dn-pro-password" style={{ fontSize: '12px', fontWeight: 600, color: 'rgba(255,255,255,0.8)', letterSpacing: '0.05em' }}>PASSWORD</label>
              <div style={{ position: 'relative' }}>
                <span style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)', fontSize: '16px', pointerEvents: 'none' }}>🔒</span>
                <input
                  id="dn-pro-password"
